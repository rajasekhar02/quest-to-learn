...
const renderEducationDetailsCollapses = function (educationDetails, stringSlug) {
  return educationDetails.map((eachEducationDetail) => {
    const getEduKeyValue = (key, defaultValue = undefined) => get(eachEducationDetail, key, defaultValue);
    return (
      <div className="list-group" key={getEduKeyValue('educationSlug')}>
        ...
        >
          <div className="d-flex w-100 justify-content-between">
            <h5 className="mb-1">{get(eachEducationDetail, 'universityName')}</h5>
            <small
              className={`${get(eachEducationDetail, 'isCurrent', false) && 'fw-bolder fs-6'}`}
            >
              {`${formatDate(
        ...
        <div
          className={`collapse ${[get(eachEducationDetail, 'educationSlug'), undefined].includes(
            stringSlug,
          )} ${
            get(eachEducationDetail, 'coursesEnrolledCollection.items', []).length !== 0 && 'show'
          }`}
        >
          <div className="card card-body">
            {get(eachEducationDetail, 'coursesEnrolledCollection.items', []).map((eachCourse) => (
              <details key={eachCourse.title}>
                <summary>{eachCourse.title}</summary>
                {documentToReactComponents(get(eachCourse, 'courseDescription.json'))}
              </details>
            ))}
          </div>
        </div>
...
const renderEducationDetailsPlaceHolder = function () {
  return [1, 2].map((listItem) => (
    <div className="list-group placeholder-glow" aria-hidden="true" key={listItem}>
      <div
        // href={`/about-me/${get(eachEducationDetail, "educationSlug")}`}
        role="button"
        data-toggle="collapse-mst-study"
      ...
      >
        ...
        <span className="mt-2 placeholder col-4" />
      </div>
